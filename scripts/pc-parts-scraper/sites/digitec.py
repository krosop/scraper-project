...
class DigitecScraper:
    """Scraper for Digitec DZ — WooCommerce store."""
        ...
        for card in soup.select('li.product, div.product'):
            try:
                ...
                if not name_el:
                    ...
                    name_el = card.select_one('a h2, a .woocommerce-loop-product__title')
                    if name_el:
                        name_el = name_el.find_parent('a')
                if not name_el:
                    continue
                ...
                # Image
                img_el = card.select_one('img.attachment-woocommerce_thumbnail, img.wp-post-image')
                image = ''
                if img_el:
                ...
                # Availability
                avail = 'In stock'
                if card.select_one('.out-of-stock, .sold-out'):
                    avail = 'Out of stock'
    ...
    def scrape_category(self, url: str, name: str) -> List[Dict]:
        print(f"[+] Digitec scraping {name}: {url}")
        all_products = []
    ...
    def scrape_all(self, categories: list = None) -> List[Dict]:
        ...
        print(f"\n[+] Digitec: {len(all_products)} products total")
        return all_products
