...
function detectCategoryFromName(name) {
  ...
  // RTX product names:  GPU, NOT a laptop (unless it has explicit laptop model numbers like G15, G16, etc.)
  // ROG and ROG Strix are GPU brands, not laptop indicators by themselves
  const hasGPU = /\brtx\s*\d{3,4}\b|\bgtx\s*\d{3,4}\b|\brx\s*\d{4}\b|\bgeforce\s+rtx\b|\bgeforce\s+gtx\b|\bradeon\s+rx\b/.test(lower);
  const laptopIndicators = /\blaptop\b|\bnotebook\b|\bpc\s+portable\b|\bordinateur\s+portable\b/;
  const laptopModel = /\b(?:g15|g16|g18|g14|g17|g513|g733)\b/; // ROG laptop model numbers
  if (hasGPU && !laptopIndicators.test(lower) && !laptopModel.test(lower)) {
    return 'graphics-cards';
  }
  
  // LAPTOP check — explicit laptop keywords only (ROG / ROG Strix is a GPU brand, NOT a laptop indicator)
  const laptopKeywords = /\bpavilion\b|\bomen\b|\blegion\b|\bzephyrus\b|\bthinkpad\b|\bideapad\b|\bvictus\b|\bnitro\b|\bpredator\b|\balienware\b|\bxps\b|\blatitude\b|\binspiron\b|\bsurface\b|\bmacbook\b|\bchromebook\b|\byoga\b|\bswift\b|\baspire\b|\bstealth\b|\brazer\b|\bblade\b|\bdefender\b|\berazer\b/;
  if (laptopKeywords.test(lower) || laptopIndicators.test(lower) || laptopModel.test(lower)) return 'pc-parts';
